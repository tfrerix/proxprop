...
def compute_loss_and_accuracy(model, loss_fn, x, y, batch_size=64, cuda=False):
    ...
    for sample_x, sample_y in loader:
        num_batches += 1
        if cuda:
            sample_x = Variable(sample_x.cuda(), requires_grad=False)
            sample_y = Variable(sample_y.cuda(), requires_grad=False)
        else:
            sample_x = Variable(sample_x, requires_grad=False)
            sample_y = Variable(sample_y, requires_grad=False)
        sample_out = model(sample_x)
        loss += loss_fn(sample_out, sample_y).data[0]
        _, y_pred = sample_out.max(dim=1)
        correct_samples  += sample_y.numel() - torch.nonzero(y_pred.data - sample_y.data).numel()
    acc = float(correct_samples) / float(num_samples)
    loss = float(loss) / float(num_batches)
    model.train(True)
    return loss, acc

def train(model, loss_fn, optimizer, data, num_epochs, batch_size, scheduler=None, cuda=False):
    ...
    epoch_full_batch_loss, epoch_full_batch_training_acc = compute_loss_and_accuracy(model, loss_fn, x_train, y_train, batch_size=batch_size, cuda=cuda)
    val_loss, val_acc = compute_loss_and_accuracy(model, loss_fn, x_val, y_val, batch_size=batch_size, cuda=cuda)
    ...
    for epoch in range(num_epochs):
        ...
        epoch_full_batch_loss, epoch_full_batch_training_acc = compute_loss_and_accuracy(model, loss_fn, x_train, y_train, batch_size=batch_size, cuda=cuda)
        val_loss, val_acc = compute_loss_and_accuracy(model, loss_fn, x_val, y_val, cuda=cuda)
    ...
    test_loss, test_acc = compute_loss_and_accuracy(model, loss_fn, x_test, y_test, batch_size=256, cuda=cuda)
